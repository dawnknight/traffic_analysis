...
car_var = []
ped_var = []
im1 = np.zeros([750,900,3])
im2 = np.zeros([750,900,3])

for i in range(len(imlist)-dt):
    print i
    im1 = np.array(Image.open(imlist[i+dt])).astype(np.float)
    im2 = np.array(Image.open(imlist[i])).astype(np.float)
    diff = im1[0:,0:,1]-im2[0:,0:,1]
...
#plot(range(len(L1_var)),ped_var, color = 'black',lw=2)

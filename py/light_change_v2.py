...
#    car =[[962,689],[1048,876]]  #Night
    ...
    def Tra_Sub_Ana(conn,sub_imlist,sub_imlist2,nstart,ip,nrow,ncol,nband):
        ...
        L1_cut_R = np.zeros([L1[3]-L1[1],L1[2]-L1[0]])
        L2_cut_R = np.zeros([L2[3]-L2[1],L2[2]-L2[0]])
        car_cut_R = np.zeros([car[3]-car[1],car[2]-car[0]])
        env_sub_R = np.zeros([nrow,ncol])        

        L1_cut_G = np.zeros([L1[3]-L1[1],L1[2]-L1[0]])
        L2_cut_G = np.zeros([L2[3]-L2[1],L2[2]-L2[0]])
        car_cut_G = np.zeros([car[3]-car[1],car[2]-car[0]])
        env_sub_G =np.zeros([nrow,ncol])
        
        L1_cut_B = np.zeros([L1[3]-L1[1],L1[2]-L1[0]])
        L2_cut_B = np.zeros([L2[3]-L2[1],L2[2]-L2[0]])
        car_cut_B = np.zeros([car[3]-car[1],car[2]-car[0]])
        ...
        for i in range(len(zip(sub_imlist,sub_imlist2))) : 
            ...
            L1_cut_R[:,:]  = diff[L1[1]:L1[3],L1[0]:L1[2],0]
            L2_cut_R[:,:]  = diff[L2[1]:L2[3],L2[0]:L2[2],0]
            car_cut_R[:,:] = diff[car[1]:car[3],car[0]:car[2],0]
            env_sub_R[:,:] = diff[:,:,0]

#======================================================================

            L1_cut_G[:,:]  = diff[L1[1]:L1[3],L1[0]:L1[2],1]
            L2_cut_G[:,:]  = diff[L2[1]:L2[3],L2[0]:L2[2],1]
            car_cut_G[:,:] = diff[car[1]:car[3],car[0]:car[2],1]
            env_sub_G[:,:] = diff[:,:,1]

#========================================================================
            L1_cut_B[:,:]  = diff[L1[1]:L1[3],L1[0]:L1[2],2]
            L2_cut_B[:,:]  = diff[L2[1]:L2[3],L2[0]:L2[2],2]
            car_cut_B[:,:] = diff[car[1]:car[3],car[0]:car[2],2]
            env_sub_B[:,:] = diff[:,:,2]
...
def main():
    coor_path = Io()
    coordinate = pickle.load(open(coor_path,"rb"))
    L1_var,L2_var,car_var,env_var,L1_avg,L2_avg = Tra_Ana(16,coordinate) 
